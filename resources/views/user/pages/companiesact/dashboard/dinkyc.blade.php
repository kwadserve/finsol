@if(count($userDinkycDetails)>0)<b>Dinkyc Details</b>
<hr />
<table class="table table-condensed table-striped">
    ...
    <tbody>
        @if($userDinkycDetails)
        @foreach($userDinkycDetails as $key => $detail)
        <tr class="align-middle" data-toggle="collapse" data-target="#{{$detail->type.$key}}" class="accordion-toggle">
                ...
                @if ($detail->payment_status == 'Credit')
                    ...
                    @else
                        <form action="{{ route('dashboard.register') }}" method="POST">
                            ...
                            <input type="hidden" name="payment_amount" value="{{ $dinkyc_instamojo_amount }}">
            ...
            <td colspan=7>
                ...
                <span class="badge badge rounded-pill d-block p-2 badge-subtle-warning"
                    data-bs-toggle="collapse" data-bs-target="#collapseContent2">Query
