@if (count($userTdsDetails) > 0)<b>Adt Details</b>
    <hr />
    <table class="table table-condensed table-striped">
        <thead>
            <tr>
                <th scope="col">Name Of TDS</th>
        ...
        <tbody>
            @if ($userTdsDetails)
                @foreach ($userTdsDetails as $key => $detail)
                    <tr class="align-middle" data-toggle="collapse" data-target="#{{ $detail->type . $key }}"
                        class="accordion-toggle">

                        <td class="text-nowrap">{{ isset($detail->name_of_tds) ? $detail->name_of_tds : '-' }}
                        </td>
                            ...
                            @if ($detail->payment_status == 'Credit')
                                ...
                                @else
                                    <form action="{{ route('dashboard.register') }}" method="POST">
                                        ...
                                        <input type="hidden" name="payment_amount" value="{{ $tds_instamojo_amount }}">
                                        <input type="hidden" name="route" value="form_dashboard">
                                        <input type="hidden" name="payment_purpose" value="Tds">
