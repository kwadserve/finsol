@if(count($userImportExportDetails)>0)<b>IMPORTEXPORT Details</b><hr/>
<table class="table table-condensed table-striped">
                                                            ...
                                                            <tbody>
                                                                @if($userImportExportDetails)
                                                                @foreach($userImportExportDetails as $detail)
                                                                <tr class="align-middle" data-toggle="collapse"
                                                                    data-target="#{{$detail->type}}"
                                                                    class="accordion-toggle">

                                                                    <td class="text-nowrap">{{isset($detail->name_of_firm)?$detail->name_of_firm:'-'}}</td>

                                                                    <td class="text-nowrap">
                                                                        <div class="align-items-center">

                                                                            <div class="ms-2">{{$detail->firm_email}}
                                                                            </div>
                                                                        </div>
                                                                    </td>
               ...
               <form action="{{ route('dashboard.register') }}" method="POST">
                   ...
                   <input type="hidden" name="payment_amount" value="{{$import_instamojo_amount}}">
                   <input type="hidden" name="route" value="form_dashboard">
                   <input type="hidden" name="payment_purpose" value="Import">
                   <input type="hidden" name="email_id" value="{{$detail->firm_email}}">
                   <input type="hidden" name="name_of_pan" value="{{$detail->name_of_firm}}">
                   <input type="hidden" name="mobile_number" value="{{$detail->firm_mobile}}">
