@if (count($userTaxauditDetails) > 0)<b>TAXAUDIT Details</b>
    ...
    <table class="table table-condensed table-striped">
        ...
        <tbody>
            @if ($userTaxauditDetails)
                @foreach ($userTaxauditDetails as $key => $detail)
                    <tr class="align-middle" data-toggle="collapse"
                        data-target="#{{ $detail->type . $key }}"
                        class="accordion-toggle">

                        <td class="text-nowrap">{{ isset($detail->name_of_taxaudit) ? $detail->name_of_taxaudit : '-' }}
                        </td>
                            ...
                            @if ($detail->payment_status == 'Credit')
                                ...
                                @else
                                    <form action="{{ route('dashboard.register') }}" method="POST">
                                        ...
                                        <input type="hidden" name="payment_amount" value="{{ $taxAudit_instamojo_amount }}">
                                        <input type="hidden" name="route" value="form_dashboard">
                                        <input type="hidden" name="payment_purpose" value="Tax Audit">
                                        <input type="hidden" name="email_id" value="{{ $detail->email_id }}">
                                        <input type="hidden" name="name_of_pan" value="{{ $detail->name_of_taxaudit }}">
