@if($usersFssai) 
<div class="row">  <h5>FSSAI Form Details</h5>
                            <div class="col-12">
                                <div id="tableExample"
                                    ...
                                    @if (session('additionalfilenotexist_fssai'))
                                                        <div class="alert alert-danger border-2 d-flex align-items-center"
                                                            ...
                                                            <p class="mb-0 flex-1">{{ session('additionalfilenotexist_fssai') }}</p>
                                        ...
                                        <table class="table table-bordered table-striped fs--1 mb-0">
                                            ...
                                            <tbody class="list">
                                                ...
                                                <tr class="align-middle" data-toggle="collapse"  class="accordion-toggle">
                                                    ...
                                                    <td><a href="{{ url('admin/user/forms/details/fssai/'.$detail->id) }}">Details</a></td>
                                                    <td>
                                                    ...
                                                    <td colspan=6 style="display:flex;">
                                                        <span class="btn btn-info ml-1 mb-1 btn-sm" title="Add Note"
                                                            type="button"  onclick="openFssaiNoteModal({{ $detail->id }})"
                                                           href="{{ url('forms/statusview/'.$detail->id) }}" data-target="#myFssaiNoteModal">
                                                            Note<span class="glyphicon glyphicon-eye-open ms-1"></span>
                                                        </span> 

 
                                                         <span class="btn btn-success ml-1 mb-1 btn-sm  " title="Change Status"
                                                            type="button" data-toggle="modal"  onclick="openFssaiApproveModal({{ $detail->id }})"
                                                             
                                                            data-target="#myFssaiApprovedModal"> 
                                                            Approve<span
                        ...
                        <script>
                                                         var urlpath="{{ $routeurl }}";
                                                            function openFssaiNoteModal(itemId) {
                                                                ...
                                                                $.ajax({
                                                                    url: urlpath+'/user/forms/statusview' +'?for=note&formtype=fssai&id=' + itemId,
                                                                    type: 'GET',
                                                            ...
                                                            function openFssaiApproveModal(itemId) {
                                                                // Make an AJAX GET request to fetch the item details
                                                                $.ajax({
                                                                    url:  urlpath+'/user/forms/statusview' +'?for=approve&formtype=fssai&id=' + itemId,
                                                                    type: 'GET',
                                                                    success: function (data) {
