@if($usersFactoryLicense) 
<div class="row">  
    <h5>FACTORY LICENSE Form Details</h5>
                            <div class="col-12">
                                <div id="tableExample"
                                    ...
                                    <div class="table-responsive scrollbar">
                                        ...
                                        <table class="table table-bordered table-striped fs--1 mb-0">
                                            ...
                                            <tbody class="list">
                                                @if($usersFactoryLicense)
                                                @foreach($usersFactoryLicense as $detail)
                                                <tr class="align-middle" data-toggle="collapse"  class="accordion-toggle">
                        
                                                    <td class="text-nowrap">{{($detail->name_of_firm)?$detail->name_of_firm:'--'}}</td>
                                                    ...
                                                    <td><a href="{{ url('admin/user/forms/details/factorylicense/'.$detail->id) }}">Details</a></td>
                                                    ...
                                                    <td colspan=6 style="display:flex;">
                                                        <span class="btn btn-info ml-1 mb-1 btn-sm" title="Add Note"
                                                            type="button"  onclick="openFactoryLicenseNoteModal({{ $detail->id }})"
                                                           href="{{ url('forms/statusview/'.$detail->id) }}" data-target="#myFactoryLicenseNoteModal">
                                                            Note<span class="glyphicon glyphicon-eye-open ms-1"></span>
                                                        </span> 

 
                                                         <span class="btn btn-success ml-1 mb-1 btn-sm  " title="Change Status"
                                                            type="button" data-toggle="modal"  onclick="openFactoryLicenseApproveModal({{ $detail->id }})"
                                                             
                                                            data-target="#myFactoryLicenseApprovedModal"> 
                                                            Approve<span
                        ...
                        <script>
                                                         var urlpath="{{ $routeurl }}";
                                                            function openFactoryLicenseNoteModal(itemId) {
                                                                ...
                                                                $.ajax({
                                                                    url: urlpath+'/user/forms/statusview' +'?for=note&formtype=factorylicense&id=' + itemId,
                                                                    type: 'GET',
                                                            ...
                                                            function openFactoryLicenseApproveModal(itemId) {
                                                                // Make an AJAX GET request to fetch the item details
                                                                $.ajax({
                                                                    url:  urlpath+'/user/forms/statusview' +'?for=approve&formtype=factorylicense&id=' + itemId,
                                                                    type: 'GET',
                                                                    success: function (data) {
