                                ...
                                <div id="tableExample"
                                    ...
                                    <div class="table-responsive scrollbar">
                                        ...
                                        <table class="table table-bordered table-striped fs--1 mb-0">
                                            ...
                                            <tbody class="list">
                                                @if($usersTrademark)
                                                @foreach($usersTrademark as $detail)
                                                <tr class="align-middle" data-toggle="collapse"  class="accordion-toggle">
                        
                                                    <td class="text-nowrap">{{($detail->name_of_trademark)?$detail->name_of_trademark:'--'}}</td>
                                                    ...
                                                    <td><a href="{{ url('admin/user/forms/details/trademark/'.$detail->id) }}">Details</a></td>
                                                    <td>
                                                    ...
                                                    <td colspan=6 style="display:flex;">
                                                        <span class="btn btn-info ml-1 mb-1 btn-sm" title="Add Note"
                                                            type="button"  onclick="openTrademarkNoteModal({{ $detail->id }})"
                                                           href="{{ url('forms/statusview/'.$detail->id) }}" data-target="#myTrademarkNoteModal">
                                                            Note<span class="glyphicon glyphicon-eye-open ms-1"></span>
                                                        </span> 

 
                                                         <span class="btn btn-success ml-1 mb-1 btn-sm  " title="Change Status"
                                                            type="button" data-toggle="modal"  onclick="openTrademarkApproveModal({{ $detail->id }})"
                                                             
                                                            data-target="#myTrademarkApprovedModal"> 
                                                            Approve<span
                        ...
                        <script>
                                                         var urlpath="{{ $routeurl }}";
                                                            function openTrademarkNoteModal(itemId) {
                                                                ...
                                                                $.ajax({
                                                                    url: urlpath+'/user/forms/statusview' +'?for=note&formtype=trademark&id=' + itemId,
                                                                    type: 'GET',
                                                            ...
                                                            function openTrademarkApproveModal(itemId) {
                                                                // Make an AJAX GET request to fetch the item details
                                                                $.ajax({
                                                                    url:  urlpath+'/user/forms/statusview' +'?for=approve&formtype=trademark&id=' + itemId,
                                                                    type: 'GET',
                                                                    success: function (data) {
