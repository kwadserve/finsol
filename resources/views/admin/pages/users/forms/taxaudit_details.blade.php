@if($usersTaxaudit) 
<div class="row">  <h5>TAXAUDIT Form Details</h5>
                            <div class="col-12">
                                <div id="tableExample"
                                    data-list='{"valueNames":["name","email","age"],"page":5,"pagination":true}'>
                                    <div class="table-responsive scrollbar">
                                    @if (session('additionalfilenotexist_taxaudit'))
                                                        <div class="alert alert-danger border-2 d-flex align-items-center"
                                                            role="alert">
                                                            <div class="bg-danger me-3 icon-item"><span
                                                                    class="fas fa-check-circle text-white fs-3"></span>
                                                            </div>
                                                            <p class="mb-0 flex-1">{{ session('additionalfilenotexist_taxaudit') }}</p>
                                        ...
                                        <table class="table table-bordered table-striped fs--1 mb-0">
                                            ...
                                            <tbody class="list">
                                                @if($usersTaxaudit)
                                                @foreach($usersTaxaudit as $detail)
                                                <tr class="align-middle" data-toggle="collapse"  class="accordion-toggle">
                                                    ...
                                                    <td><a href="{{ url('admin/user/forms/details/taxaudit/'.$detail->id) }}">Details</a></td>
                                                    <td>
                                                    ...
                                                    <td colspan=6 style="display:flex;">
                                                        <span class="btn btn-info ml-1 mb-1 btn-sm" title="Add Note"
                                                            type="button"  onclick="openTaxauditNoteModal({{ $detail->id }})"
                                                           href="{{ url('forms/statusview/'.$detail->id) }}" data-target="#myTaxauditNoteModal">
                                                            Note<span class="glyphicon glyphicon-eye-open ms-1"></span>
                                                        </span> 

 
                                                         <span class="btn btn-success ml-1 mb-1 btn-sm  " title="Change Status"
                                                            type="button" data-toggle="modal"  onclick="openTaxauditApproveModal({{ $detail->id }})"
                                                             
                                                            data-target="#myTaxauditApprovedModal"> 
                                                            Approve<span
                        ...
                        <script>
                                                         var urlpath="{{ $routeurl }}";
                                                            function openTaxauditNoteModal(itemId) {
                                                                ...
                                                                $.ajax({
                                                                    url: urlpath+'/user/forms/statusview' +'?for=note&formtype=taxaudit&id=' + itemId,
                                                                    type: 'GET',
                                                            ...
                                                            function openTaxauditApproveModal(itemId) {
                                                                // Make an AJAX GET request to fetch the item details
                                                                $.ajax({
                                                                    url:  urlpath+'/user/forms/statusview' +'?for=approve&formtype=taxaudit&id=' + itemId,
                                                                    type: 'GET',
                                                                    success: function (data) {
