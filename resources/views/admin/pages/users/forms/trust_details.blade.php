        ...
        <div id="tableExample" data-list='{"valueNames":["name","email","age"],"page":5,"pagination":true}'>
            <div class="table-responsive scrollbar">
                <table class="table table-bordered table-striped fs--1 mb-0">
                    ...
                    <tbody class="list">
                        @if($usersTrust)
                        @foreach($usersTrust as $detail)
                        <tr class="align-middle" data-toggle="collapse" class="accordion-toggle">

                            <td class="text-nowrap">{{($detail->name_of_trust)?$detail->name_of_trust:'--'}}</td>
                            ...
                            <td><a href="{{ url('admin/user/forms/details/trust/'.$detail->id) }}">Details</a></td>
                            <td>
                            ...
                            <td>
                                ...
                                <div><span
                                    ...
                                    <form action="{{ url('admin/user/forms/additional/file/' . $detail->user_id) }}"
                                        method="POST">
                                        @csrf
                                ...
                                <form action="{{ url('admin/user/forms/approved/file/' . $detail->user_id) }}"
                                    method="POST">
                                    @csrf
                                    <input type="hidden" name="form_type" value="Trust">
                            ...
                            <td colspan=6 style="display:flex;">
                                <span class="btn btn-info ml-1 mb-1 btn-sm" title="Add Note"
                                                            type="button"  onclick="openTrustNoteModal({{ $detail->id }})"
                                                           href="{{ url('forms/statusview/'.$detail->id) }}" data-target="#myTrustNoteModal">
                                                            Note<span class="glyphicon glyphicon-eye-open ms-1"></span>
                                </span>


                                <span class="btn btn-success ml-1 mb-1 btn-sm  " title="Change Status"
                                                            type="button" data-toggle="modal"  onclick="openTrustApproveModal({{ $detail->id }})"

                                                            data-target="#myTrustApprovedModal">
                                                            Approve<span
...
<script>
    var urlpath="{{ $routeurl }}";
                                                            function openTrustNoteModal(itemId) {
                                                                ...
                                                                $.ajax({
                                                                    url: urlpath+'/user/forms/statusview' +'?for=note&formtype=trust&id=' + itemId,
                                                                    type: 'GET',
                                                            ...
                                                            function openTrustApproveModal(itemId) {
                                                                // Make an AJAX GET request to fetch the item details
                                                                $.ajax({
                                                                    url:  urlpath+'/user/forms/statusview' +'?for=approve&formtype=trust&id=' + itemId,
                                                                    type: 'GET',
                                                                    success: function (data) {
