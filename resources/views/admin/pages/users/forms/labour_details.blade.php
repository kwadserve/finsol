    ...
    <h5>LABOUR Form Details</h5>
                                ...
                                <div id="tableExample"
                                    ...
                                    <div class="table-responsive scrollbar">
                                        ...
                                        <table class="table table-bordered table-striped fs--1 mb-0">
                                            ...
                                            <tbody class="list">
                                                @if($usersLabour)
                                                @foreach($usersLabour as $detail)
                                                <tr class="align-middle" data-toggle="collapse"  class="accordion-toggle">
                        
                                                    <td class="text-nowrap">{{($detail->name_of_labour)?$detail->name_of_labour:'--'}}</td>
                                                    ...
                                                    <td><a href="{{ url('admin/user/forms/details/labour/'.$detail->id) }}">Details</a></td>
                                                    <td>
                                                    ...
                                                    <td colspan=6 style="display:flex;">
                                                        <span class="btn btn-info ml-1 mb-1 btn-sm" title="Add Note"
                                                            type="button"  onclick="openLabourNoteModal({{ $detail->id }})"
                                                           href="{{ url('forms/statusview/'.$detail->id) }}" data-target="#myLabourNoteModal">
                                                            Note<span class="glyphicon glyphicon-eye-open ms-1"></span>
                                                        </span> 

 
                                                         <span class="btn btn-success ml-1 mb-1 btn-sm  " title="Change Status"
                                                            type="button" data-toggle="modal"  onclick="openLabourApproveModal({{ $detail->id }})"
                                                             
                                                            data-target="#myLabourApprovedModal"> 
                                                            Approve<span
                        ...
                        <script>
                                                         var urlpath="{{ $routeurl }}";
                                                            function openLabourNoteModal(itemId) {
                                                                // Make an AJAX GET request to fetch the item details
                                                                $.ajax({
                                                                    url: urlpath+'/user/forms/statusview' +'?for=note&formtype=labour&id=' + itemId,
                                                                    type: 'GET',
                                                            ...
                                                            function openLabourApproveModal(itemId) {
                                                                // Make an AJAX GET request to fetch the item details
                                                                $.ajax({
                                                                    url:  urlpath+'/user/forms/statusview' +'?for=approve&formtype=labour&id=' + itemId,
                                                                    type: 'GET',
                                                                    success: function (data) {
