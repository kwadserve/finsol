@if($usersStatutoryaudit) 
<div class="row">  <h5>Statutory Audit Form Details</h5>
                            <div class="col-12">
                                <div id="tableExample"
                                    ...
                                    <div class="table-responsive scrollbar">
                                        <table class="table table-bordered table-striped fs--1 mb-0">
                                            ...
                                            <tbody class="list">
                                                @if($usersStatutoryaudit)
                                                @foreach($usersStatutoryaudit as $detail)
                                                <tr class="align-middle">
                                                    ...
                                                    <td><a href="{{ url('admin/user/companiesact/details/statutoryaudit'.$detail->id) }}">Details</a></td>
                                                    <td>
                                                        @if($detail->status == 2)
                                                        <div><span
                                                    ...
                                                    <td colspan=6 style="display:flex;">
                                                        <span class="btn btn-info ml-1 mb-1 btn-sm" title="Add Note"
                                                            type="button"  onclick="openStatutoryauditNoteModal({{ $detail->id }})"
                                                           href="{{ url('companiesact/statusview/'.$detail->id) }}" data-target="#myStatutoryauditNoteModal">
                                                            Note<span class="glyphicon glyphicon-eye-open ms-1"></span>
                                                        </span> 

 
                                                         <span class="btn btn-success ml-1 mb-1 btn-sm  " title="Change Status"
                                                            type="button" data-toggle="modal"  onclick="openStatutoryauditApproveModal({{ $detail->id }})"
                                                             
                                                            data-target="#myStatutoryauditApprovedModal"> 
                                                            Approve<span
                        ...
                        <script>
                                                         var urlpath="{{ $routeurl }}";
                                                            function openStatutoryauditNoteModal(itemId) {
                                                                ...
                                                                $.ajax({
                                                                    ...
                                                                    success: function (data) {
                                                                        ...
                                                                        // $('#myStatutoryauditNoteModal #userid').val(data.user_id);
                                                                        // $('#myStatutoryauditNoteModal #panid').val(data.id);
                                                                        // $('#myStatutoryauditNoteModal #routeis').val('pan');
                                                                    },
                                                                    error: function (xhr) {
                                                                        // Handle error cases
                                                            ...
                                                            function openStatutoryauditApproveModal(itemId) {
                                                                ...
                                                                $.ajax({
                                                                    // url:  urlpath+'/user/forms/statusview' +'?pan=' + itemId,
                                                                    url: urlpath+'/user/companiesact/statusview' +'?for=approve&formtype=statutoryaudit&id=' + itemId,
                                                                    type: 'GET',
                                                                    success: function (data) {
                                
                                                                        $('#myCaCommonApprovedModal').modal('show');
                                                                        $('#ca-approve-modal-body-div').html(data.modalBody);
                                                                        // $('#myStatutoryauditApprovedModal #userid').val(data.user_id);
                                                                        // $('#myStatutoryauditApprovedModal #panid').val(data.id);
                                                                        // $('#myStatutoryauditApprovedModal #nameofpan').val(data.name_of_pan);
                                                                        // $('#myStatutoryauditApprovedModal #type').val('approve');
                                                                    },
                                                                    error: function (xhr) {
                                                                        // Handle error cases
                                                                        console.log(xhr.responseText);
