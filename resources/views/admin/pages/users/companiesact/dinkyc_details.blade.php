                    ...
                    <tbody class="list">
                        ...
                        <tr class="align-middle">
                            ...
                            <td>
                                ...
                                <form action="{{ url('admin/user/companiesact/approved/file/' . $detail->user_id) }}"
                                    method="POST">
                                    @csrf
                                    <input type="hidden" name="form_type" value="Dinkyc">
                            ...
                            <td colspan=6 style="display:flex;">
                                <span class="btn btn-info ml-1 mb-1 btn-sm" title="Add Note" type="button"
                                ...
                                <span class="btn btn-success ml-1 mb-1 btn-sm  " title="Change Status" type="button"
                                    data-toggle="modal" onclick="openDinkycApproveModal({{ $detail->id }})"
...
function openDinkycNoteModal(itemId) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // $('#myDinkycNoteModal #userid').val(data.user_id);
            // $('#myDinkycNoteModal #panid').val(data.id);
            // $('#myDinkycNoteModal #routeis').val('pan');
        },
        error: function(xhr) {
            // Handle error cases
...
function openDinkycApproveModal(itemId) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // $('#myDinkycApprovedModal #userid').val(data.user_id);
            // $('#myDinkycApprovedModal #panid').val(data.id);
            // $('#myDinkycApprovedModal #nameofpan').val(data.name_of_pan);
            // $('#myDinkycApprovedModal #type').val('approve');
        },
        error: function(xhr) {
            // Handle error cases
            console.log(xhr.responseText);
